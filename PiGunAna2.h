...
// Mon Sep 25 17:07:52 2023 by ROOT version 6.18/04
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   Int_t           ncah;
   Int_t           caori[150];   //[ncah]
   Int_t           caci0[150];   //[ncah]
   Int_t           caci1[150];   //[ncah]
   Float_t         capox[150];   //[ncah]
   Float_t         capoy[150];   //[ncah]
   Float_t         capoz[150];   //[ncah]
   Float_t         caene[150];   //[ncah]
   Float_t         catim[150];   //[ncah]
   Int_t           nvt;
   Int_t           vtori[1];   //[nvt]
   Int_t           vtpri[1];   //[nvt]
   Int_t           vtrpl[1];   //[nvt]
...
#ifdef PiGunAna2_cxx
PiGunAna2::PiGunAna2(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("lctuple_example.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("lctuple_piGunPt10Theta10_170.root");
      }
...
Int_t PiGunAna2::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t PiGunAna2::LoadTree(Long64_t entry)
{
...
void PiGunAna2::Init(TTree *tree)
{
...
void PiGunAna2::Show(Long64_t entry)
{
...
Int_t PiGunAna2::Cut(Long64_t entry)
{
